...
class TpnUnionPaysController extends Controller
{
    private $method     = 'unionpay';
    private $payment    = '';
    ...
    public function unionpay()
    {
        ...
        if ($transaction_state == $this->payment->getTransactionState($this->method) && $status == 'success') {
            ...
            return view('pages.tpn-union-index', [
                'data' => $data,
            ]);
        }

        /** redirect to not success page */
        return view('welcome');
    }

    public function unionpay_notify(Request $request)
    {
        $payload    = request()->all();

        $attributes['type'] = 'UnionPay';
    ...
    public function unionpay_callback()
    {
        $payload = request()->all();
        ...
        if ($same_id && $status == 'true') {
            // if ($same_id && $status == 'true' && $trans->checksum_status == 's') { //this will work only notify come before callback
        ...
        if( $this->isolate==false ) {
            ...
            return view('pages.tpn-union-result', [
                'msg' => $msg,
            ]);
        }
    }
