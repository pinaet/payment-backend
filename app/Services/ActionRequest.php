...
abstract class ActionRequest
{
    ...
    public function __construct()
    {
        ...
        $this->client = new Client([
            'base_uri' => env('2C2P_BASE_URI'),
            'handler' => $handler
        ]);


        // JWS setup
        $jwsAlgorithmManager = new AlgorithmManager([new PS256()]);
        $this->jwsBuilder = new JWSBuilder($jwsAlgorithmManager);
        $this->jwsVerifier = new JWSVerifier($jwsAlgorithmManager);
        $this->jwsCompactSerializer = new JWSCompactSerializer();

        $jwsHeaderCheckerManager = new HeaderCheckerManager(
            [new AlgorithmChecker(['PS256'])],
            [new JWSTokenSupport()]
        );

        $this->jwsLoader = new JWSLoader(
            new JWSSerializerManager([$this->jwsCompactSerializer]),
            $this->jwsVerifier,
            $jwsHeaderCheckerManager
        );


        // JWE setup
        $jweKeyAlgorithmManager = new AlgorithmManager([new RSAOAEP()]);
        $jweContentAlgorithmManager = new AlgorithmManager([new A128CBCHS256()]);
        $this->jweBuilder = new JWEBuilder(
            $jweKeyAlgorithmManager,
            $jweContentAlgorithmManager,
        ...
        $jweHeaderCheckerManager = new HeaderCheckerManager(
            [new AlgorithmChecker([SecurityData::$JWEAlgorithm, SecurityData::$JWEEncrptionAlgorithm])],
            [new JWETokenSupport()]
        );

        $this->jweLoader = new JWELoader(
            new JWESerializerManager([$this->jweCompactSerializer]),
            $this->jweDecrypter,
            $jweHeaderCheckerManager
        );
